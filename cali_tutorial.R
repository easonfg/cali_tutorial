...
pipe_run = function(ninstances){
  ...
  #train with 'train' dataset and and predict 'test' dataset with svm
  svmfit = svm(y ~ ., data = train, kernel = "linear", cost = 10, scale = FALSE, probability=TRUE)
  ygrid = predict(svmfit, test[,-length(test)], probability=TRUE)
  ygrid_norm = as.data.frame(ygrid + (abs(min(ygrid)) + 0.00001))
  ### train and predict  ### 
